...
const apiUrl = "https://api-v2.gptdao.ai";
const event_id = "genai_202405";

const getSponsors = async () => {
  const res = await fetch(
    `${apiUrl}/exhibitors?encrypted_event_id=KdHOj6Unpa2GL9KnA2W7vTw-rOncshmAjqwynIookOE%3D`,
  );
  if (!res.ok) {
  ...
  for (const date of dates) {
    try {
      ...
      const sessions = agenda.data.results.map((session) => ({
        ...
        moderators: [],
        type: session.tags.length > 0 ? session.tags[0] : "Other",
        room: session.place,
      }));
