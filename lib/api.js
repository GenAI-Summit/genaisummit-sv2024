...
const getNotableAttendees = async () => {
  const fetchAttendees = async (type) => {
    const res = await fetch(
      `https://api.gptdao.ai/additional/list?partnership_type=${type}`,
    );
    if (!res.ok) {
...
const getHomeSpeakers = async () => {
  let res = await fetch("https://api.gptdao.ai/home/speaker");
  if (!res.ok) {
...
const getHomeExhibitors = async () => {
  let res = await fetch("https://api.gptdao.ai/home/exhibitor");
  if (!res.ok) {
