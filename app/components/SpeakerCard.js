...
const SpeakerCard = ({ speaker }) => {
  ...
  return (
    <>
      <div
        className="group relative h-auto w-20 md:w-36 md:h-auto bg-white border-2 border-white hover:border-sky-500 ease-in-out duration-300 overflow-hidden"
        onClick={openDrawer}
      >
        <Image
          src={speaker.image}
        ...
        <div className="h-15">
          ...
          <div className="absolute bottom-0 w-full bg-white bg-opacity-80 p-0.5 text-center transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out">
            <div className="text-xs font-medium">{speaker.title}</div>
            <div className="text-xs font-medium">{speaker.company}</div>
          </div>
        </div>
      </div>
